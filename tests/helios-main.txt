...
$script:ModulesToLoad = @(
    # Foundation Modules (no dependencies, loaded first for core functionality)
    @{ Name = "exceptions"; Path = "modules\exceptions.psm1"; Required = $true },
    @{ Name = "logger"; Path = "modules\logger.psm1"; Required = $true },

    # Core Application Modules (depend on foundation)
    @{ Name = "event-system"; Path = "modules\event-system.psm1"; Required = $true },
    @{ Name = "tui-engine"; Path = "modules\tui-engine.psm1"; Required = $true }, # Manages $global:TuiState

    # UI Modules (depend on tui-engine)
    ...
    # Service Modules (depend on event-system, potentially tui-engine for UI interactions)
        ...
        foreach ($screenName in $script:ScreenModules) {
            ...
            try {
                if (Test-Path $screenPath) {
                    ...
                    # Determine the screen factory function name based on convention
                    $factoryFunctionName = "Get-" + ((($screenName -split "-") | ForEach-Object { 
                        $_.Substring(0,1).ToUpper() + $_.Substring(1) 
                    }) -join "") + "Screen" # e.g., Get-DashboardScreen, Get-TaskScreen
